 ...
 * hovers, is a proxy.
 */
public class KernelHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public KernelHyperlinkDetector(Resource resource) {
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelHyperlink(new Region(offset, length), resolvedEObject, text);
