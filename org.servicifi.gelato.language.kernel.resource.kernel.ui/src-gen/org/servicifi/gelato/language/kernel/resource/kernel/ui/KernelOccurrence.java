 ...
 */
public class KernelOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(org.servicifi.gelato.language.kernel.resource.kernel.ui.IKernelTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.servicifi.gelato.language.kernel.resource.kernel.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.servicifi.gelato.language.kernel.resource.kernel.ui.occurences.declaration";
	
	private final static org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelPositionHelper positionHelper = new org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelPositionHelper();
	 ...
	 */
	private org.servicifi.gelato.language.kernel.resource.kernel.IKernelTextResource textResource;
	...
	/**
	 * <p>
	 * Creates a KernelOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public KernelOccurrence(org.servicifi.gelato.language.kernel.resource.kernel.IKernelTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(org.servicifi.gelato.language.kernel.resource.kernel.IKernelLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		org.servicifi.gelato.language.kernel.resource.kernel.ui.IKernelTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new org.servicifi.gelato.language.kernel.resource.kernel.analysis.KernelDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(org.servicifi.gelato.language.kernel.resource.kernel.ui.IKernelTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		org.servicifi.gelato.language.kernel.resource.kernel.ui.IKernelTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected org.servicifi.gelato.language.kernel.resource.kernel.ui.IKernelTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		org.servicifi.gelato.language.kernel.resource.kernel.IKernelLocationMap locationMap = textResource.getLocationMap();
		
		org.servicifi.gelato.language.kernel.resource.kernel.ui.IKernelTokenScanner tokenScanner = createTokenScanner();
	...
	protected org.servicifi.gelato.language.kernel.resource.kernel.ui.IKernelTokenScanner createTokenScanner() {
		return new org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelPositionCategory type, String text, int offset, int length) {
		...
		if (type == org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.servicifi.gelato.language.kernel.resource.kernel.ui.KernelOccurrence.DECLARATION_ANNOTATION_ID);
	}
