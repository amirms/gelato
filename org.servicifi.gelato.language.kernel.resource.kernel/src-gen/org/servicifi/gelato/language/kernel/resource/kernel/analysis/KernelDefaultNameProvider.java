...
public class KernelDefaultNameProvider implements org.servicifi.gelato.language.kernel.resource.kernel.IKernelNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.servicifi.gelato.language.kernel.resource.kernel.util.KernelEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
