...
public class KernelLayoutUtil {
	...
	public void transferLayoutInformationToModel(EObject element) {
		...
		for (Iterator<org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(EObject element) {
		...
		if (layoutReference != null) {
			...
			for (Iterator<EObject> i = list.iterator(); i.hasNext(); ) {
				...
				org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public EObject createLayoutInformationModelElement(org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLayoutInformation layoutInformation, EPackage layoutPackage) {
		EFactory factory = layoutPackage.getEFactoryInstance();
		Object object = layoutInformation.getObject(null, false);
		org.servicifi.gelato.language.kernel.resource.kernel.grammar.KernelSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), org.servicifi.gelato.language.kernel.resource.kernel.grammar.KernelGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLayoutInformation createLayoutInformation(EObject layoutInformationModelElement) {
		Object object = null;
		...
		org.servicifi.gelato.language.kernel.resource.kernel.grammar.KernelSyntaxElement syntaxElement = org.servicifi.gelato.language.kernel.resource.kernel.grammar.KernelGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof org.servicifi.gelato.language.kernel.resource.kernel.grammar.KernelPlaceholder) {
			org.servicifi.gelato.language.kernel.resource.kernel.grammar.KernelPlaceholder placeholder = (org.servicifi.gelato.language.kernel.resource.kernel.grammar.KernelPlaceholder) syntaxElement;
			org.servicifi.gelato.language.kernel.resource.kernel.IKernelTokenResolver tokenResolver = new org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			org.servicifi.gelato.language.kernel.resource.kernel.IKernelTokenResolveResult result = new org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLayoutInformationAdapter getLayoutInformationAdapter(EObject element) {
	...
	public void removeLayoutInformationAdapter(EObject element) {
		org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLayoutInformationAdapter existingAdapter = getLayoutInformationAdapter(element);
