 ...
 */
public class KernelMinimalModelHelper {
	
	private final static org.servicifi.gelato.language.kernel.resource.kernel.util.KernelEClassUtil eClassUtil = new org.servicifi.gelato.language.kernel.resource.kernel.util.KernelEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.servicifi.gelato.language.kernel.resource.kernel.util.KernelStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = org.servicifi.gelato.language.kernel.resource.kernel.util.KernelListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.servicifi.gelato.language.kernel.resource.kernel.util.KernelStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = org.servicifi.gelato.language.kernel.resource.kernel.util.KernelListUtil.castListUnchecked(value);
