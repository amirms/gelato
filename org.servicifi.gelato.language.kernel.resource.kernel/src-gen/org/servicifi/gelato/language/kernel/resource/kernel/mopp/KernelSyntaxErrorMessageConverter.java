...
public class KernelSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public KernelSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new org.servicifi.gelato.language.kernel.resource.kernel.mopp.KernelLocalizedMessage(message, position, line, 1, 5);
		}
	}
