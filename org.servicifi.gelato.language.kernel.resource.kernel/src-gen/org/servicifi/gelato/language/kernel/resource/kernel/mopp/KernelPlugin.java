 ...
 */
public class KernelPlugin extends Plugin {
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, KernelPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, KernelPlugin.PLUGIN_ID, message);
		}
