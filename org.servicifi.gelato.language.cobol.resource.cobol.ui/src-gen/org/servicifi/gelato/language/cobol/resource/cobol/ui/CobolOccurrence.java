...
public class CobolOccurrence {
	...
	private org.servicifi.gelato.language.cobol.resource.cobol.ui.CobolTokenScanner tokenScanner;
	 ...
	 */
	public CobolOccurrence(org.servicifi.gelato.language.cobol.resource.cobol.ICobolTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.servicifi.gelato.language.cobol.resource.cobol.ui.CobolTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.servicifi.gelato.language.cobol.resource.cobol.mopp.CobolMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.servicifi.gelato.language.cobol.resource.cobol.ui.CobolBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.servicifi.gelato.language.cobol.resource.cobol.ui.CobolPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, org.servicifi.gelato.language.cobol.resource.cobol.ui.CobolPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.servicifi.gelato.language.cobol.resource.cobol.ui.CobolPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.servicifi.gelato.language.cobol.resource.cobol.ui.CobolPositionCategory.DEFINTION) {
