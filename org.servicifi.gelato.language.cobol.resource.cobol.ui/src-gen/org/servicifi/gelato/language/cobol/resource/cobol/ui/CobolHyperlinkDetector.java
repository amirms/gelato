 ...
 * hovers, is a proxy.
 */
public class CobolHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public CobolHyperlinkDetector(Resource resource) {
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new org.servicifi.gelato.language.cobol.resource.cobol.ui.CobolHyperlink(new Region(offset, length), resolvedEObject, text);
