 ...
 */
public class PreprocessOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.IPreprocessTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.occurences.declaration";
	...
	/**
	 * <p>
	 * Creates a PreprocessOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public PreprocessOccurrence(org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.IPreprocessTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.IPreprocessLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.IPreprocessTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.PreprocessPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.analysis.PreprocessDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.IPreprocessTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.PreprocessPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.PreprocessPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.IPreprocessTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.IPreprocessTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.IPreprocessLocationMap locationMap = textResource.getLocationMap();
		
		org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.IPreprocessTokenScanner tokenScanner = createTokenScanner();
	...
	protected org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.IPreprocessTokenScanner createTokenScanner() {
		return new org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.PreprocessUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.PreprocessPositionCategory type, String text, int offset, int length) {
		...
		if (type == org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.PreprocessPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.PreprocessOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.servicifi.gelato.language.cobol.preprocess.resource.preprocess.ui.PreprocessOccurrence.DECLARATION_ANNOTATION_ID);
	}
