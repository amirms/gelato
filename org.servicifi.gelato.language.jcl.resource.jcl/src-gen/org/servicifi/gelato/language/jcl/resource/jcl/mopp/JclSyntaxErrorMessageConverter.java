...
public class JclSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public JclSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = org.servicifi.gelato.language.jcl.resource.jcl.util.JclStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
