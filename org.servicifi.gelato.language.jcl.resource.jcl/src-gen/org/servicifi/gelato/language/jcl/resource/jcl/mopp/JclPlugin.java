 ...
 */
public class JclPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "org.servicifi.gelato.language.jcl.resource.jcl";
	/**
	...
	private static JclPlugin plugin;
	...
	public static JclPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, JclPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, JclPlugin.PLUGIN_ID, message);
		}
		final JclPlugin pluginInstance = JclPlugin.getDefault();
