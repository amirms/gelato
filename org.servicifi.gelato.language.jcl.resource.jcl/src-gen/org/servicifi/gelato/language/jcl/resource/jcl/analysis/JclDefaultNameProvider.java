...
public class JclDefaultNameProvider implements org.servicifi.gelato.language.jcl.resource.jcl.IJclNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.servicifi.gelato.language.jcl.resource.jcl.util.JclEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
