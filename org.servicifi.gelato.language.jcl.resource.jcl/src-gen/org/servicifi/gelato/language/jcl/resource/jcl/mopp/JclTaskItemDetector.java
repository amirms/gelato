...
/**
 * The JclTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * JclTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class JclTaskItemDetector {
	...
	public List<org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclTaskItem> findTaskItems(String text, int line, int charStart) {
		List<org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclTaskItem> foundItems = new ArrayList<org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclTaskItem(keyword, message, localLine, offset, end));
