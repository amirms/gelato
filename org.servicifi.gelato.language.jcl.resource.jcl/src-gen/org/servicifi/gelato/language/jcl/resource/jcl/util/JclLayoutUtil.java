...
public class JclLayoutUtil {
	...
	public void transferLayoutInformationToModel(EObject element) {
		...
		for (Iterator<org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(EObject element) {
		...
		if (layoutReference != null) {
			...
			for (Iterator<EObject> i = list.iterator(); i.hasNext(); ) {
				...
				org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public EObject createLayoutInformationModelElement(org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLayoutInformation layoutInformation, EPackage layoutPackage) {
		EFactory factory = layoutPackage.getEFactoryInstance();
		Object object = layoutInformation.getObject(null, false);
		org.servicifi.gelato.language.jcl.resource.jcl.grammar.JclSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), org.servicifi.gelato.language.jcl.resource.jcl.grammar.JclGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLayoutInformation createLayoutInformation(EObject layoutInformationModelElement) {
		Object object = null;
		...
		org.servicifi.gelato.language.jcl.resource.jcl.grammar.JclSyntaxElement syntaxElement = org.servicifi.gelato.language.jcl.resource.jcl.grammar.JclGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof org.servicifi.gelato.language.jcl.resource.jcl.grammar.JclPlaceholder) {
			org.servicifi.gelato.language.jcl.resource.jcl.grammar.JclPlaceholder placeholder = (org.servicifi.gelato.language.jcl.resource.jcl.grammar.JclPlaceholder) syntaxElement;
			org.servicifi.gelato.language.jcl.resource.jcl.IJclTokenResolver tokenResolver = new org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			org.servicifi.gelato.language.jcl.resource.jcl.IJclTokenResolveResult result = new org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLayoutInformationAdapter getLayoutInformationAdapter(EObject element) {
	...
	public void removeLayoutInformationAdapter(EObject element) {
		org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclLayoutInformationAdapter existingAdapter = getLayoutInformationAdapter(element);
