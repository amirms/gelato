...
public class JclOccurrence {
	...
	private org.servicifi.gelato.language.jcl.resource.jcl.ui.JclTokenScanner tokenScanner;
	 ...
	 */
	public JclOccurrence(org.servicifi.gelato.language.jcl.resource.jcl.IJclTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.servicifi.gelato.language.jcl.resource.jcl.ui.JclTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.servicifi.gelato.language.jcl.resource.jcl.mopp.JclMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.servicifi.gelato.language.jcl.resource.jcl.ui.JclBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.servicifi.gelato.language.jcl.resource.jcl.ui.JclPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, org.servicifi.gelato.language.jcl.resource.jcl.ui.JclPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.servicifi.gelato.language.jcl.resource.jcl.ui.JclPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.servicifi.gelato.language.jcl.resource.jcl.ui.JclPositionCategory.DEFINTION) {
