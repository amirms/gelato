 ...
 * hovers, is a proxy.
 */
public class JclHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public JclHyperlinkDetector(Resource resource) {
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new org.servicifi.gelato.language.jcl.resource.jcl.ui.JclHyperlink(new Region(offset, length), resolvedEObject, text);
