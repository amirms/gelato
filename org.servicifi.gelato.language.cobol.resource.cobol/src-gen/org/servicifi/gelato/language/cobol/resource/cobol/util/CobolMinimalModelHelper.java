 ...
 */
public class CobolMinimalModelHelper {
	
	private final static org.servicifi.gelato.language.cobol.resource.cobol.util.CobolEClassUtil eClassUtil = new org.servicifi.gelato.language.cobol.resource.cobol.util.CobolEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.servicifi.gelato.language.cobol.resource.cobol.util.CobolStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = org.servicifi.gelato.language.cobol.resource.cobol.util.CobolListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.servicifi.gelato.language.cobol.resource.cobol.util.CobolStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = org.servicifi.gelato.language.cobol.resource.cobol.util.CobolListUtil.castListUnchecked(value);
