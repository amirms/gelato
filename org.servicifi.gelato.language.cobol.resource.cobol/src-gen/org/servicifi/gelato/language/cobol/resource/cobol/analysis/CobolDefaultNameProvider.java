...
public class CobolDefaultNameProvider implements org.servicifi.gelato.language.cobol.resource.cobol.ICobolNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.servicifi.gelato.language.cobol.resource.cobol.util.CobolEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
