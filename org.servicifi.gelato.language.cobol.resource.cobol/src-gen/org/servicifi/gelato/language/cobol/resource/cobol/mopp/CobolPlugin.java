 ...
 */
public class CobolPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "org.servicifi.gelato.language.cobol.resource.cobol";
	/**
	...
	private static CobolPlugin plugin;
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, CobolPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, CobolPlugin.PLUGIN_ID, message);
		}
		final CobolPlugin pluginInstance = CobolPlugin.getDefault();
