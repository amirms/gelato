...
/**
 * The CobolTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * CobolTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class CobolTaskItemDetector {
	...
	public List<org.servicifi.gelato.language.cobol.resource.cobol.mopp.CobolTaskItem> findTaskItems(String text, int line, int charStart) {
		List<org.servicifi.gelato.language.cobol.resource.cobol.mopp.CobolTaskItem> foundItems = new ArrayList<org.servicifi.gelato.language.cobol.resource.cobol.mopp.CobolTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.servicifi.gelato.language.cobol.resource.cobol.mopp.CobolTaskItem(keyword, message, localLine, offset, end));
