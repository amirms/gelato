 ...
 */
public class CommentOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(org.servicifi.gelato.language.cobol.comment.resource.comment.ui.ICommentTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.servicifi.gelato.language.cobol.comment.resource.comment.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.servicifi.gelato.language.cobol.comment.resource.comment.ui.occurences.declaration";
	
	private final static org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentPositionHelper positionHelper = new org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentPositionHelper();
	 ...
	 */
	private org.servicifi.gelato.language.cobol.comment.resource.comment.ICommentTextResource textResource;
	...
	/**
	 * <p>
	 * Creates a CommentOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public CommentOccurrence(org.servicifi.gelato.language.cobol.comment.resource.comment.ICommentTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(org.servicifi.gelato.language.cobol.comment.resource.comment.ICommentLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		org.servicifi.gelato.language.cobol.comment.resource.comment.ui.ICommentTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new org.servicifi.gelato.language.cobol.comment.resource.comment.analysis.CommentDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(org.servicifi.gelato.language.cobol.comment.resource.comment.ui.ICommentTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		org.servicifi.gelato.language.cobol.comment.resource.comment.ui.ICommentTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected org.servicifi.gelato.language.cobol.comment.resource.comment.ui.ICommentTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		org.servicifi.gelato.language.cobol.comment.resource.comment.ICommentLocationMap locationMap = textResource.getLocationMap();
		
		org.servicifi.gelato.language.cobol.comment.resource.comment.ui.ICommentTokenScanner tokenScanner = createTokenScanner();
	...
	protected org.servicifi.gelato.language.cobol.comment.resource.comment.ui.ICommentTokenScanner createTokenScanner() {
		return new org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentPositionCategory type, String text, int offset, int length) {
		...
		if (type == org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentOccurrence.DECLARATION_ANNOTATION_ID);
	}
