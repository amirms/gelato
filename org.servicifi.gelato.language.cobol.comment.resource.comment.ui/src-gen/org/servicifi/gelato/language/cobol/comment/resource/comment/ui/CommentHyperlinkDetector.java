 ...
 * hovers, is a proxy.
 */
public class CommentHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public CommentHyperlinkDetector(Resource resource) {
		textResource = (org.servicifi.gelato.language.cobol.comment.resource.comment.ICommentTextResource) resource;
	}
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new org.servicifi.gelato.language.cobol.comment.resource.comment.ui.CommentHyperlink(new Region(offset, length), resolvedEObject, text);
